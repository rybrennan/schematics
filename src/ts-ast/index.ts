...
export function tsAst(options: TsAstOptions): Rule {
  return (tree: Tree, _context: SchematicContext) => {
    ...
    console.log('i am ', nodes.length);
    const interfaceDeclaration = nodes.find(
      (n) => n.kind === ts.SyntaxKind.InterfaceDeclaration
    );
    ...
    const [openBrace, closeBrace] = [
      interfaceDeclaration!
        .getChildren()
        .find((n) => n.kind === ts.SyntaxKind.OpenBraceToken),
      interfaceDeclaration!
        .getChildren()
        .slice()
        .reverse()
